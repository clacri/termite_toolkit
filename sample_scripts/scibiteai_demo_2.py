...
sbclient = scibiteai.SciBiteAIClient(scibite_ai_credentials=scibite_ai_credentials,
...
# load a model
sbclient.load_model('relex', 'ppi-1400')

# predict whether a sentence contains a given relationship
sentence = 'BRCA1 binds to BRCA2'
pred = sbclient.relex_from_sent('ppi-1400', sentence)
print('\nDoes \'%s\' contain a protein-protein interaction?' % sentence)
print(pred)

# if you try to predict with an unloaded model, it will be loaded automatically
pred = sbclient.ner_from_sent('gene', sentence)
print('\n', pred)
...
print('\nThe most similar words to \'lung\' are...', sims)